...
\begin{theorem}
	...
	$$
		\{(A_1^\lambda \wedge\dots\wedge A_n^\lambda)\to(B_1^\lambda \vee\dots\vee B_n^\lambda)\:|\:\lambda\in\Lambda\}
	$$
	for $(A_1 \wedge\dots\wedge A_n)\to(B_1\vee\dots\vee B_n)\in\mathcal R$ and $\mathcal X^\#$ as the union over all sets
	$$
		\{({A^\lambda\psi} \to A^{\lambda\psi})\to(C^\lambda)\:|\:\lambda\in\Lambda, \psi\text{ does not occur in }\lambda\}
	$$
	for $\psi = (A\to B)\to C\in\mathcal X$ as well as $P^\#$ as the union over all sets
	$$
		\{A^\lambda\to A^{\lambda\psi}\:|\:\lambda\in\Lambda, \psi\psi\text{ does not occur in }\lambda\}
	$$
