...
\end{itemize}
\textbf{Contributions.} We now overview the main results of this paper (which we discuss in more detail in Sections~\ref{sec:overview} and~\ref
\begin{itemize}
	\item We establish a small model property for counter-models to validity in intuitionistic propositional logic.
	\item Based on the small model property, we provide an embedding of intuitionistic propositional logic into classical propositional logic and into quantified boolean formulas, i.e., we provide an 
	\item We discuss the reduction of counter-models in intuitionistic first-order logic and, based on this reduction, present an embedding intuitionistic into classical first-order logic.
	\item We provide an experimental evaluation on the use of provers for classical first-order logic for determining intuitionistic validity.
...
Most relevant are the embedding of intuitionistic logic into S4~\cite{basicprooftheory} and of various non-classical logics into higher-order classical logic~\cite{LEO,wisniewski2016tptp} \todo{why are they relevant?}.
Of special relevance is the reduction of intuitionistic propositional logic given in~\cite{claessen2015sat}
\todo{reduction to what?} and examined in~\cite{fiorentini2019proof} \todo{what has been examined here?}, which we further expand upon in this paper.

\todo{This paragraph is more about realizability, right? So remove, or connect to paper?}
...
\begin{wrapfigure}[13]{r}{0.4\textwidth}
	...
	\hspace*{1cm}
	\begin{tikzpicture}
		\begin{scope}[opacity=0.3, blend mode=multiply]
	...
	\end{tikzpicture}
	\vspace*{-7.1cm}
	\caption{Symmetric difference. Instances proven by Embedding (red), Embedding (idem.) (green), ileanCoP or nanoCoP (blue)}
	\label{fig:c}
\end{wrapfigure}
