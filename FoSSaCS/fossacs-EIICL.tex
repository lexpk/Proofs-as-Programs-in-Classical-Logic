...
\begin{document}
	...
	\maketitle
\begin{abstract}
The famous double negation translation~\cite{glivenko1929quelques,godel1933intuitionistischen} establishes an embedding from classical into intuitionistic logic. Curiously the reverse direction has not been covered in literature. Based on the work in~\cite{claessen2015sat} we establish a small model property for intuitionistic propositional logic in order to utilize its Kripke semantics to give a simple embedding into classical propositional logic and quantified boolean formulas. We transfer there methods to the first order case and also give an embedding of intuitionistic first-order logic into classical first-order-logic.
...
For certain classes of benchmarks our approach can compete with state-of-the-art provers, in particular when the intuitionistic content is low. Crucially, our constructions support the transfer of counter-models to validity, which is important to model checking applications, where the output of counter-examples is an appreciated feature.
\end{abstract}
...
\begin{itemize}
	...
	\item \resizebox*{0.96\hsize}{!}{Embedding intuitionistic propositional logic into quantified boolean formulas.}
