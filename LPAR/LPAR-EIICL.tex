...
Since we could not find a proof of this, one is given in the appendix~\ref{proof:first-order-normal-form}.
...
The idea is to use the Kripke semantics for intuitionistic logic, expressed in classical logic, to perform an embedding of formulas. That is, for each $\varphi$ we give a $\varphi^\#$ that is classically valid iff $\varphi$ is intuitionistically valid, establishing a dual to the double negation translation.
