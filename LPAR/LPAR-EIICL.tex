...
\begin{definition}
	A \emph{Kripke structure}  \sout{$\mathcal K = (W, (v_w)_{w\in W})$} \red{$\mathcal K = (\mathbf W, (v_w)_{w\in W})$} consists of a partially ordered set $\mathbf W = (W, \leq)$ of \emph{worlds}, called the Kripke \emph{frame}, and an indexed family of valuations $(v_w)_{w\in W}$ such that \sout{$v_u(A)\leq v_w(A)$} \red{$v_u(A) = 1$ entails $v_w(A) = 1$} for all worlds $u\leq w$ and propositional variables $A$ (this is called the \emph{persistency condition}).
...
\begin{theorem}\label{thm:prop-translation}
	...
	with flat $\mathcal R$ and implicational $\mathcal S$. Let $\Lambda$ denote the set of sequences without repetition over $\mathcal X$. For every propositional variable $A$ and $\lambda\in\Lambda$ consider a new propositional \sout{Variable}\red{variable} $A^\lambda$. Define $\mathcal R^\#$ as the union over all sets
	$$
...
The translation from the last section will serve as \red{a} starting point for this section.
...
\begin{definition}\label{def:fo-translation}
	...
	\begin{itemize}
		...
		\item $(\varphi\circ\psi)^b = \varphi^b\circ\sout{\psi^u}\red{\psi^b}$ for $\circ\in\{\wedge, \vee\}$.
		...
		\item $(\forall x\varphi)^b = \forall w(\sout{i}\red{b}\preceq w\to \forall x(E(x, w)\to \varphi^w))$ where $w$ is a new bound variable.
	\end{itemize}
...
$$\forall w\forall\vec x(s\preceq w\to\vec E(\vec x, w)\to \forall y(E(y, f_\psi(\vec x, w))\to A(\vec a, f_\psi(\vec x, w)))\to B(\vec{\red{b}}, w))$$
with $f_\psi$ being a new function symbol.
...
Even if we \sout{don't}\red{don't} have the finiteness property of the propositional case, we have still managed to reduce the complexity of possible counter-models because we know that considering worlds of the form $$g_{\psi_1}(\vec m_1, g_{\psi_2}(\vec m_2, \dots g_{\psi_n}(\vec m_n, s)\dots))$$ is sufficient.
...
we observe good performance of our approach for $|\mathcal Q| = 0$ and $|\mathcal X| \leq 2$\sout{resp. $|\mathcal X| \geq 6$}.
