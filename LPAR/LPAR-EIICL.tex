...
For propositional logic we assume an ambient set of propositional variables. As usual a formula is either $\bot$, a propositional variable or, given formulas $\varphi$ and $\psi$ and $\circ\in\{\wedge, \vee,\to\}$, $\varphi\circ\psi$.

For predicate logic we assume an ambient signature $\Sigma$, i.e., a set of function and predicate symbols with their respective arities, and a set of variables. A term is either a variable or, given an $n$-ary function symbol $f$ and terms $t_1, \ldots, t_n$, $f(t_1, \ldots, t_n)$. A formula is either $\bot$, or, given a $n$-ary predicate symbol $P$ and terms $t_1, \ldots, t_n$, $P(t_1, \ldots, t_n)$, or, given formulas $\varphi$ and $\psi$ and $\circ\in\{\wedge, \vee,\to\}$, $\varphi\circ\psi$, or, given a formula $\varphi$ and a variable $x$, $\exists x\varphi$ or $\forall x\varphi$. Free and bound variables are defined as usual and there is a Barendregt's convention in place, i.e. we implicitly assume that they comprise distinct sets, such that bound variables are not affected by substitutions of free variables.
...
\begin{definition}
	...
	\end{itemize}
	 Here $a$ is a free variable that does not occur in $\varphi$. $\varphi$ is satisfiable if $\mathcal M, v\models\varphi$ for some $\mathcal M, v$. It is valid if $\mathcal M, v\models\varphi$ for every $\mathcal M, v$. We denote the set of valid formulas with \emph{CQC} (Classical Quantified Calculus).
\end{definition}
...
\begin{definition}
	...
	We denote the set of valid formulas with \emph{IQC} (Intuitionistic Quantified Calculus).
\end{definition}
...
=======
		...
		\item $(\exists x\varphi)^S_Z = \varphi[s(z_1,\dots,z_n)/x]^S_Z$ where $s$ is a new function, $\{z_1\dots z_n\} = Z$.\\$(\exists x\varphi)^H_Z = \exists x(\varphi[a/x]^H_{Z\cup\{a\}}[x/a])$
 where $a$ is a new free variable.
>>>>>>> c8d6f4a7fa194cc08089d36f91f2f60b706bd539
	\end{itemize}
...
\begin{lemma}
	For each propositional formula $\varphi$ there is effectively a QBF $\varphi^Q$ \todo{Ich würde die Formeln $\varphi^Q$ hier und in Section 6 aus Symmetriegründen auch $\varphi^\#$ nennen.} with $\mathcal |\varphi^Q|\in\mathcal O(|\mathcal R|\cdot|\mathcal X| + |\mathcal X|^3)$ such that $\varphi$ is intuitionistically invalid if and only if $\varphi^Q$ is a satisfiable QBF.
\end{lemma}
...
\begin{definition}
	Let $\varphi$ be a propositional formula.\todo{genau genommen müsstest du noch sagen, dass $\nu$ eine freie Variable ist. Im letzten Fall der Definition benützen wir dann aber w gleichzeitig als freie und gebundene Variable}
 Define $\varphi^{\nu}$ inductively:
	\vspace*{-.1cm}
	\begin{itemize}
		\item $\bot^\nu = \bot$.
...
\begin{definition}
For $\psi\in\mathcal X$ let $[\psi]$ denote the corresponding clause in $\mathcal X^{b S}$. Suppose we are given a counter-model $\mathcal M = (M, I)$ for $\varphi^{CH}$ such that $m\preceq f_\psi^I(m)$ for each $m\in M$, $\psi\in \mathcal X$.
...
\begin{lemma}
	Suppose $(M, T)$ is a counter-model to $\varphi^{CH}$, then so is $(M_T, I_T)$.
\end{lemma}
\begin{proof}
	It follows directly from the definitions that $M_T, I_T\not\models P(s)$ and $M_T, I_T\models K(\varphi)$. It is easy to show that $M_T, I_T\models \psi$ for $\psi\in\mathcal R^{bS}$ from persistency. Let $\psi\in\mathcal X^{bS}$, we show $M_T, I_T[\psi_1\dots\psi_n/w] \models s\preceq w\to (w\preceq f_\psi(w)\to A(f_\psi(w)\to B(f_\psi(w)))\to C(w))$ for all $\psi_1\dots\psi_n\in M_T$. Let $\psi_1\dots\psi_n\in M_T$ and $m = g_{\psi_n}(\dots(g_{\psi_1}(s^I))$. If $M, I[m/w]\models C(w)$ we are done. Otherwise $M, I[m/w]\not\models s\preceq w$ or $M, I[m/w]\not\models w\preceq f_\psi(w)\to A(f_\psi(w)) \to B(f_\psi(w))$. The former is impossible since $g_\psi$ is increasing for all $\psi$. For the latter to hold we must have $M, I[m/w]\models w\preceq f_\psi(w)$, $M, I[m/w]\models A(f_\psi(w))$ and $M, I[m/w]\not\models B(f_\psi(w))$. If $\psi$ is fulfilled at $m$ we have $g_\psi(m) = m$, otherwise $g_\psi(m) = f_{[\psi]}^I(m)$. In any case $M, I'[\psi_1\dots\psi_n/w]\models A(f_\psi(w))$ and $M_T, I_T[\psi_1\dots\psi_n/w]\not\models B(f_\psi(w))$. It follows that $M_T, I_T[\psi_1\dots\psi_n/w]\not\models w\preceq f_\psi(w)\to A(f_\psi(w))\to B(f_\psi(w))$ and therefore $M_T, I_T\models \psi$.
=======
	1. If $C^I(m)$ is true then we are done due to persistency. Otherwise $C^I(m)$ is false.
	Because $(A^I(f_{[\psi]}^I(m))\to B^I(f_{[\psi]}^I(m)))\to C^I(m)$ holds, then $A^I(f_{[\psi]}^I(m))\to B^I(f_{[\psi]}^I(m))$ must be false.
...
\begin{theorem}\label{thm:prop-translation}
	...
	with flat $\mathcal R$ and implication $\mathcal S$. Let $\Lambda$ denote the set of sequences without repetition over $\mathcal X$. For every propositional variable $A$ and $\lambda\in\Lambda$ consider a new propositional Variable $A^\lambda$. Define $\mathcal R^\#$ as the union over all sets
	$$
	...
	$$
	for $\psi = (A\to B)\to C\in\mathcal X$ as well as $P^\#$ \todo{Hier ist nur P statt mathcal P, was aus Symmetriegründen vielleicht hilfreich wäre, aber P wird auch schon für die finale Präposition verwendet, also wäre ein anderer Buchstabe wohl besser} as the union over all sets
	$$
...
\todo{Wollen wir noch etwas sagen wie ``However the previous PSPACE result does not support the transfer of countermodels that we develop in this section''?}
...
\todo{genau genommen müsstest du noch sagen, dass $\nu$ eine freie Variable ist. Im letzten Fall der Definition benützen wir dann aber w gleichzeitig als freie und gebundene Variable}
	\begin{itemize}
		\item $\bot^\nu = \bot$.
...
Then, $(\bigwedge\mathcal R\wedge\bigwedge\mathcal X)\to P$ has an intuitionistic counter-model. By the previous section\todo{unklar was mit "previous section" gemeint ist, und was für ein model du genau meinst.} there exists a classical counter-model $\mathcal M$ for $(\bigwedge\mathcal R^\#\wedge\bigwedge\mathcal X^\#)\to P^\epsilon$.
...
Let $W_0 = \{w\in W\:|\: w_0\preceq w\}$ and define an equivalence relation $\sim$ on $\{(x, u)\:|\:u\in W_0, x\in M_u\}$ via $(x, u)\sim (y, w)$ iff $x = y$ and there exists $v\in W_0$ comparable with both $u, w$ such that $x\in v$ and denote the equivalence class of $(x, u)$ with $[x, u]$. Let $M = W_0\cup \{[x, u]\:|\:u\in W_0, x\in M_u\}$.
	...
	\emph{Claim.} For any formula $\psi$ in which the free variable $b$ doesn't occur, world $w_k\in W_0$ and variable assignments $v_c, v_i$ such that $v_c(a) = [v_k(a), w_k]$ \todo{das sollte $v_i$ statt $v_k$ sein, oder} for all free variables $a$ occurring in $\psi$ it holds that
\todo{Hier und weiter und sind immer M,I genannt, aber eigentlich haben wir Notation für $\mathcal M,v \models$ definiert}
\[M, I, v_c[w_k/b]\models \psi^b\text{ if and only if $w_k, v_i\models \psi$.}\]
	...
	Suppose first that $w_k, v_i\not\models \forall x\psi'$, i.e. there exists $w_l\geq w_k$ and $m_l\in w_l$ such that $w_l, v_i[m_l/a]\not\models \psi'[a/x]$. Then by the induction hypothesis $M, I, v_c[[v_i(a), w_l],w_l/c]\not\models \psi'[a/x]^c$\todo{hier fehlt die Ersetzung für $[v_i(a), w_l]$. Fehlt außerdem nicht irgendein Bezug zu $m_l$?}. Furthermore by definition $M, I, v_c[w_l/c, w_k/b]\models b\preceq c$ and $M, I, v_c[v_c[[v_i(a), w_l],w_l/c]\models E(a, c)$\todo{hier fehlt die Ersetzung für $[v_i(a), w_l]$. Außerdem ist hier wirklich $v_c[v_c$ gemeint?}. Hence $M, I, v_c[w_k/b]\not\models \psi^b$. Suppose on the other hand that $w_k, v_i\not\models\forall x\psi'$, i.e. for all $w_l\geq w_k$ and $m_l\in w_l$ it holds that $w_l, v_i[m_l/a]\models \psi'[a/x]$. Then by the induction hypothesis $M, I, v_c[[m_l, w_l]/A,w_l/c]\models \psi'[a/x]^c$\todo{Hier wohl a statt A} for all $w_l\geq w_k$ and $m_l\in w_l$. It can be shown that for every other assignment of $c$ and $a$ either $b\preceq c$ or $E(a, c)$ is violated. Hence $M, I, v_c[w_k/b]\models \psi^b$. For other forms of $\psi$ one proceeds in similar fashion. This completes the proof of the claim.
...
\todo{Da ich hier nicht durchgekommen bin, kannst du den ganzen Beweis nochmal checken :p ?}
\end{proof}
