...
\begin{definition}
	...
	\begin{itemize}
		\item $M_T$ is the union of $\{m\in M\:|\:\text{World}^I(m)\wedge s^I\preceq^I m\}$ and sequences $(\vec m_1, \psi_1)\dots (\vec m_n, \psi_n)$ such that $\vec E^I(\vec m_1, w_0)$ and $\vec E^I(\vec m_i, g_{\psi_i}(\vec m_i, g_{\psi_{i-1}}(\vec m_{i-1}, \dots g_{\psi_1}(\vec m_1, w_0)\dots)))$ for $i > 0$ hold.
		\item $s^{I_T} = s^I$, $\vec r^{I_T} = \vec r^I$.
		\item We interpret $\preceq$ trivially on $M$ and as the prefix-order elsewhere.
		\item Have $E^{I_T}(m, (\vec m_1, \psi_1)\dots (\vec m_n, \psi_n))$ iff $E^I(m, g_{\psi_n}(\vec m_n, \dots g_{\psi_1}(\vec m_1, w_0)\dots))$.
		...
		\item For predicate symbols $R$, we set $${R}^{I_T}(\vec m, (\vec m_1, \psi_1)\dots (\vec m_n, \psi_n)) = {R}^I(\vec m, g_{\psi_n}(\vec m_n, \dots(g_{\psi_1}(\vec m_1, s^I))\dots))$$ and arbitrary otherwise, for function symbols $f^{I_T}(\vec m) = f^I(\vec m)$ and arbitrary else.
	\end{itemize}
\end{definition}
...
Even if we don't have the finiteness property of the propositional case, we have still managed to reduce the complexity of possible counter-models because we know that considering worlds of the form $$g_{\psi_1}(\vec m_1, g_{\psi_2}(\vec m_2, \dots g_{\psi_n}(\vec m_n, s)\dots))$$ is sufficient.
