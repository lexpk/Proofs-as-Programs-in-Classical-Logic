...
\end{definition}
\todo{I think with the implicit forall quanitifcation over the free variables in our semantics, now also in the skolem case the free variables should be universally quantified}
\begin{theorem}
...
\end{itemize}
and by removing removing the $s\preceq u$ for the remaining formulas in $\mathcal S^\circ$.
...
\todo{we already define $\varphi^\circ$, shouldn't this be some $\varphi^\dagger$? Also the phrase before should be: Then,$\varphi^\circ$ is equivalid to ...?}
As in the previous section we can define a tree counter-model for this.

\begin{definition}
	Suppose we are given a counter-model $\mathcal M = (M, I)$ for $\varphi^\circ$. \todo{shouldn't this be $\varphi^\dagger$?} Define $\mathcal M_T = (M_T, I_T)$ as follows:
	\begin{itemize}
...
\begin{lemma}
	If $\mathcal M$ is a counter-model to $\varphi^\circ$ \todo{shouldn't this be $\varphi^\dagger$?} then so is $\mathcal M_T$.
\end{lemma}
...
from $K(\varphi)$ and replace $\forall u\forall w\forall x((E(x, u)\wedge u\preceq w)\to E(x, w))$ with $\bigwedge\{\forall \vec z\forall u(E(x, u)\to E(x, f_\psi(\vec z, u)))\:|\:\psi\in\mathcal F_\to\}$ and analogously for persistency to obtain $K$. We set
