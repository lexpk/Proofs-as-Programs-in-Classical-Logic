...
\begin{theorem}\label{fullFOtranslation}
	...
	\end{itemize}
	Then $\bigwedge S\to P$ is intuitionistically iff $K\to \bigwedge\mathcal S^\#\to P^\#(s)$ is classically valid, where $s$ is a new constant symbol and
	\begin{align*}
		K := &\exists xE(x, s)\wedge \forall x\forall y\forall z(\neg(E(x, z)\wedge E(z, y)))\wedge\\
...
\begin{definition}
	...
	\begin{align*}
		...
		Persistent(u, w)=&\:\bigwedge \{A(u)\to A(w))\:|\: \text{ $A$ is a propositional variable that occurs in $\varphi$}\}
	\end{align*}
	Then define
...
First of all recall that due to Lemma~\ref{equivalid} it is sufficient to consider sentences of the form $\bigwedge\mathcal S\to P$ where all $\psi\in\mathcal S$ are of one of the forms

$$\forall \vec z_\psi(A_\psi(\vec a_\psi)\to (B_\psi(\vec b_\psi)\wedge C_\psi(\vec c_\psi))), \forall \vec z_\psi((A_\psi(\vec a_\psi)\wedge B_\psi(\vec b_\psi))\to C_\psi(\vec c_\psi)),$$$$ \forall \vec z_\psi(A_\psi(\vec a)\to (B_\psi(\vec b)\vee C_\psi(\vec c))),
\forall \vec z_\psi((A_\psi(\vec a)\vee B_\psi(\vec b))\to C_\psi(\vec c)),$$$$ \forall \vec z_\psi((A_\psi(\vec a_\psi)\to B_\psi(\vec b_\psi))\to C_\psi(\vec c_\psi)),\forall \vec z_\psi(\forall xA_\psi(\vec a_\psi)\to B_\psi(\vec b_\psi)),$$$$ \forall \vec z_\psi(A_\psi(\vec a_\psi)\to\forall xB_\psi(b_\psi)), \forall \vec z_\psi(\exists xA_\psi(\vec a_\psi)\to B_\psi(\vec b_\psi)), \forall \vec z_\psi(A_\psi(\vec a_\psi)\to\exists xB_\psi(b_\psi))$$	
...
\begin{itemize}
	\item $\forall \vec z_\psi\forall u(s\preceq u\to \vec E(\vec z_\psi, u)\to (A_\psi^\#(\vec a_\psi, u)\to (B_\psi^\#(\vec b_\psi, u)\circ C_\psi^\#(\vec c_\psi, u))))$\\for each $\circ\in\{\wedge, \vee\}, \psi = \forall \vec z_\psi(A_\psi(\vec a_\psi)\to (B_\psi(\vec b_\psi)\circ C_\psi(\vec c_\psi)))\in\mathcal S$
	\item $\forall \vec z_\psi\forall u(s\preceq u\to\vec E(\vec z_\psi, u)\to (A_\psi^\#(\vec a_\psi, u)\circ B_\psi^\#(\vec b_\psi, u))\to C_\psi^\#(\vec c_\psi, u))$\\for each $\circ\in\{\wedge, \vee\}, \psi = \forall \vec z_\psi((A_\psi(\vec a_\psi)\circ B_\psi(\vec b_\psi))\to C_\psi(\vec c_\psi))\in\mathcal S$
	\item $\forall \vec z_\psi\forall u(s\preceq u\to\vec E(\vec z_\psi, u)\to\forall w(u\preceq w\to A_\psi^\#(\vec a_\psi, w)\to B_\psi^\#(\vec b_\psi, w))\to C_\psi^\#(\vec c_\psi, u))$\\ for each $\psi = \forall \vec z_\psi((A_\psi(\vec a_\psi)\to B_\psi(\vec b_\psi))\to C_\psi(\vec c_\psi))\in\mathcal S$
	\item  $\forall \vec z_\psi\forall u(s\preceq u\to\vec E(\vec z_\psi, u)\to \forall w(u\preceq w\to \forall x(E(x, w)\to A_\psi^\#(\vec a_\psi, w)))\to B^\#(\vec b_\psi, u))$\\for each $\forall \vec z_\psi(\forall xA_\psi(\vec a_\psi)\to B_\psi(\vec t_B))\in\mathcal S$.
	\item $\forall \vec z_\psi\forall u(s\preceq u\to\vec E(\vec z_\psi, u)\to A^\#_\psi(\vec a_\psi, u)\to \forall x(E(x, u)\to B^\#_\psi(\vec b_\psi, u)))$\\for each $\forall \vec t(A_\psi(\vec a_\psi)\to \forall xB_\psi(\vec b_\psi))\in\mathcal S$.
	\item $\forall \vec z_\psi\forall u(s\preceq u\to\vec E(\vec z_\psi, u)\to \exists x(E(x, u)\wedge A^\#_\psi(\vec z_\psi, u))\to B^\#_\psi(\vec b_\psi))$\\for each $\forall \vec z_\psi(\exists xA_\psi(\vec a_\psi)\to B_\psi(\vec b_\psi))\in\mathcal S$
	\item $\forall \vec z_\psi\forall u(s\preceq u\to\vec E(\vec z_\psi, u)\to A^\#_\psi(\vec b_\psi)\to \exists x(E(x, u)\wedge B^\#_\psi(\vec b_\psi, u)))$\\for each $\forall \vec z_\psi(A_\psi(\vec a_\psi)\to \exists xB_\psi(\vec b_\psi))\in\mathcal S$
\end{itemize}
...
\begin{proof}
	...
	$$ \forall \vec z_\psi((A_\psi(\vec a_\psi)\to B_\psi(\vec b_\psi))\to C_\psi(\vec c_\psi))$$
	We have to show that
	$$M, I\models \forall \vec z_\psi\forall u(\vec E(t_\psi, u)\to \forall w(u\preceq w\to A_\psi^\#(\vec a_\psi, w)\to B_\psi^\#(\vec b_\psi, w))\to C_\psi^\#(\vec c_\psi, u))$$
	holds. Suppose towards contradiction that it does not, i.e. there are $u, \vec t_\psi\in M$ such that for all $w\in M$
	$$\vec E^I_\psi(t_\psi, u)\to (u\preceq w\to {A^\#}^I_\psi(\vec a_\psi, w)\to {B^\#}^I_\psi(\vec b_\psi, w))\to {C^\#}^I_\psi(\vec c_\psi, u)$$ is false. For that ${C^\#}^I_\psi(\vec c_\psi, u)$ must be false and in particular $u\in W_0$. Futhermore $\vec E^I_\psi(z_\psi, u)$ must be true and we can write $\vec z_\psi = [z_1, u]\dots[z_n, u]$ for some $z_i\in W_u$. Note that the above is false in particular for $w\in M_0$ with $u\preceq w$. But this would imply
	$$((A_\psi^{I_u}(\vec a_\psi)\to B_\psi^{I_u}(\vec b_\psi))\to C_\psi^{I_u}(\vec c_\psi))[\vec z_\psi/z_1\dots z_n]$$in our original Kripke counter-model. The other cases are analogous.
\end{proof}

We proceed now as in the previous section. Denote the set of formulas $\psi\in\mathcal S$ of the form $\forall\vec z_\psi((A_\psi(\vec a_\psi)\to B_\psi(\vec b_\psi))\to C_\psi(\vec c_\psi))$ with $\mathcal F_\to$ and of the form $\forall \vec z_\psi(\forall xA_\psi(\vec a_\psi)\to B_\psi(\vec b_\psi))$ with $\mathcal F_\forall$. Let $\mathcal F = \mathcal F_\to\cup\mathcal F_\forall$. For each  $\psi\in\mathcal F$ consider a new function symbol $f_\psi$. As before we may assume that $s$ is a least element and eliminate the quantification over $w$ by replacing formulas
$$\forall \vec z_\psi\forall u(s\preceq u\to\vec E(\vec z, u)\to \forall w(u\preceq w\to A_\psi^\#(\vec a_\psi, w)\to B_\psi^\#(\vec b_\psi, w))\to C_\psi^\#(\vec c_\psi, u))$$ with
$$\forall \vec z_\psi\forall u(\vec E(\vec z_\psi, u)\to (A_\psi^\#(\vec a_\psi, f_\psi(\vec t, u))\to B_\psi^\#(\vec b_\psi, f_\psi(\vec z_\psi, u)))\to C_\psi^\#(\vec c_\psi, u))$$ and
$$\forall \vec z_\psi\forall u(s\preceq u\to\vec E(\vec z_\psi, u)\to \forall w(u\preceq w\to \forall x(E(x, w)\to A_\psi^\#(\vec a_\psi, w)))\to B^\#(\vec b_\psi, u))$$ with $$\forall \vec z_\psi\forall u(\vec E(\vec z_\psi, u)\to (\forall x(E(x, f_\psi(\vec z_\psi, u))\to A_\psi^\#(\vec a_\psi, f_\psi(\vec z_\psi, u))))\to B^\#(\vec b_\psi, u))$$
...
$$\hspace*{-.5cm}\varphi^\circ= \forall u(s\preceq u)\to \bigwedge\{\forall \vec t\forall u(u\preceq f_\psi(\vec t, u))\:|\:\psi\in\mathcal F\}\to K(\varphi)\to\bigwedge\mathcal S^\#\to P^\#$$
...
Part 2 of this Lemma does not hold for $\psi\in\mathcal F_\forall$, because even if for all $x$ with $E(x, w$) we have $A(x, w)$ there could be some term $t$ with $\neg E(t, w)$ and $v$ with $w\preceq v$ such that $E(t, v)$ and $\neg A(t, w)$. This is why we must consider sequences that have repetitions of such formulas:
